...
-- Ensure Realtime is enabled for tickets
ALTER TABLE public.tickets REPLICA IDENTITY FULL;
ALTER PUBLICATION supabase_realtime ADD TABLE public.tickets;

-- Add RLS policies for role_permissions and user_permissions if missing
DO $$ 
BEGIN
  -- Check if policy exists for role_permissions
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE schemaname = 'public' 
    AND tablename = 'role_permissions' 
    AND policyname = 'Admins can view role permissions'
  ) THEN
  ...
  -- Check if user_permissions table exists and create policy
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'user_permissions' AND table_schema = 'public') THEN
    IF NOT EXISTS (
      ...
      AND policyname = 'Admins can view user permissions'
    ) THEN
