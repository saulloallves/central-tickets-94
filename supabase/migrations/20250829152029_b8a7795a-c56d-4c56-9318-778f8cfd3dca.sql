-- Criar o trigger para aplicar a função sync_message_to_conversa
...
-- Melhorar a função para buscar nomes de diferentes formas
...
BEGIN
  ...
  SELECT COALESCE(
    ...
    (SELECT f.nome_completo
     ...
     WHERE t3.id = NEW.ticket_id AND f.nome_completo IS NOT NULL
     LIMIT 1)
