-- Melhorar função de busca RAG para ser mais semântica e contextual
DROP FUNCTION IF EXISTS match_documentos(vector, float, int);

-- Criar função RAG melhorada que entende assuntos e contexto
...
-- Função compatível com nome antigo (delegando para a nova)
...
BEGIN
  ...
  SELECT
    ...
    s.categoria,
    s.versao,
    s.score_final AS similaridade
  FROM match_documentos_semantico(
    query_embedding, 
    '', 
    match_threshold, 
    match_count, 
