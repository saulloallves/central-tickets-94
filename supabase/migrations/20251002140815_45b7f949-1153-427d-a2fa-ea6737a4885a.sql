-- Corrigir tipo log_tipo na função notify_new_atendimento
...
BEGIN
  ...
  -- Registrar log do sistema com tipo correto
  PERFORM public.log_system_action(
    'sistema'::public.log_tipo,              -- tipo_log correto
    'internal_notifications',                -- entidade_afetada
    v_notif_id::text,                       -- entidade_id
    'Notificação de atendimento enviada',   -- acao_realizada
    NULL,                                   -- usuario_responsavel
    NULL,                                   -- ia_modelo
    NULL,                                   -- prompt_entrada
    NULL,                                   -- resposta_gerada
    NULL,                                   -- dados_anteriores
    jsonb_build_object(                     -- dados_novos
      'notification_id', v_notif_id,
      'chamado_id', NEW.id,
      'tipo_atendimento', NEW.tipo_atendimento,
      'is_emergencia', NEW.is_emergencia
    ),
    'painel_interno'::public.log_canal      -- canal
  );
