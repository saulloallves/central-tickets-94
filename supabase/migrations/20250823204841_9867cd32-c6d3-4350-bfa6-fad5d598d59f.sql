-- Corrigir função tickets_before_insert para que tickets 'imediato' iniciem como 'aberto'
-- Apenas tickets de 'crise' ou que já nascem com SLA vencido devem ir para 'escalonado'
...
begin
  ...
  -- Calcular status SLA - mas não escalar automaticamente tickets imediatos
  if now() >= new.data_limite_sla then
    new.status_sla := 'vencido';
    -- REMOVIDO: Não escalar automaticamente tickets imediatos na criação
    -- Apenas tickets de crise vão direto para escalonado (já tratado acima)
