-- Melhorar o trigger detect_and_group_crise para agrupar tickets similares corretamente
-- O problema é que o trigger só funciona em INSERT, mas precisa também 
-- buscar e vincular tickets similares existentes

DROP TRIGGER IF EXISTS trigger_detect_and_group_crise ON tickets;

-- Criar uma função mais robusta para detectar e agrupar crises
...
BEGIN
    ...
    IF existing_tickets_ids IS NOT NULL AND array_length(existing_tickets_ids, 1) > 0 THEN
        ...
        INSERT INTO crise_updates (
            ...
            matched_crisis_id,
            'ticket_added',
            'Crise agrupou ' || similar_tickets_count || ' tickets similares sobre: ' || primary_keyword,
            ticket_record.criado_por
        );
        
        -- Update crisis description with current count
        UPDATE crises 
        SET descricao = 'Crise automática com ' || similar_tickets_count || ' tickets sobre: ' || primary_keyword,
            updated_at = NOW()
...
CREATE TRIGGER trigger_detect_and_group_crise
    AFTER INSERT OR UPDATE ON tickets
    FOR EACH ROW
    EXECUTE FUNCTION detect_and_group_enhanced_crise();

-- Processar tickets de crise existentes que não estão vinculados
-- Buscar tickets com prioridade crise dos últimos 4 horas que não estão vinculados
WITH unlinked_crisis_tickets AS (
    SELECT t.id, t.descricao_problema, t.criado_por,
           LOWER(TRIM(SPLIT_PART(t.descricao_problema, ' ', 1))) as keyword
    FROM tickets t
    LEFT JOIN crise_ticket_links ctl ON t.id = ctl.ticket_id
    WHERE t.prioridade = 'crise'
    AND t.status IN ('escalonado', 'em_atendimento')
    AND t.data_abertura >= NOW() - INTERVAL '4 hours'
...
INSERT INTO crise_ticket_links (crise_id, ticket_id, linked_by)
SELECT ec.id, uct.id, uct.criado_por
FROM unlinked_crisis_tickets uct
JOIN existing_crises ec ON uct.keyword = ANY(ec.palavras_chave);