-- ============================================
-- CORREÇÃO DAS FUNÇÕES DE PROCESSAMENTO SLA
-- Adicionar verificação de sla_pausado = FALSE
-- ============================================

-- Corrigir process_sla_half_warnings para considerar pausas
...
BEGIN
  FOR ticket_record IN
    ...
    WHERE t.status IN ('aberto', 'em_atendimento')
      ...
      AND t.sla_pausado = FALSE  -- ✅ CORREÇÃO: Não enviar alertas para SLAs pausados
      AND NOT EXISTS (
...
-- Corrigir process_overdue_slas para considerar pausas
...
BEGIN
  FOR ticket_record IN
    ...
    WHERE t.status IN ('aberto', 'em_atendimento')
      ...
      AND t.sla_pausado = FALSE  -- ✅ CORREÇÃO: Não marcar como vencido se estiver pausado
