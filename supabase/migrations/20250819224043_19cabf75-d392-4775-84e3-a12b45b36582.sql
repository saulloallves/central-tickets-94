...
-- Inicializar positions existentes (espaçar por 1000 para permitir inserções)
UPDATE public.tickets 
SET position = (ROW_NUMBER() OVER (PARTITION BY status ORDER BY created_at)) * 1000
WHERE position = 1000;
...
CREATE TABLE IF NOT EXISTS public.tickets_audit (
  ...
  action TEXT NOT NULL, -- 'move', 'status_change', 'create', 'update'
  old_data JSONB,
  new_data JSONB,
  user_id UUID,
  ip_address INET,
