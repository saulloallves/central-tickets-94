...
/**
 * Business hours for DFCOM (extended until 18h30)
 * Returns true if current time is within DFCOM business hours (Monday-Saturday, 8h30-18h30, SP timezone)
 */
export function isDFCOMBusinessHours(): boolean {
  try {
    // Get current time in São Paulo timezone using date-fns-tz
    const now = new Date();
    const spTime = toZonedTime(now, 'America/Sao_Paulo');
    ...
    console.error("Erro ao verificar horário de funcionamento DFCOM:", error);
    // Em caso de erro, considerar como fora do horário por segurança
    return false;
  }
}
