...
serve(async (req: Request) => {
  ...
  try {
    console.log("📅 AUTOATENDIMENTO_CALENDARIO - Recebendo requisição");
    ...
    const payload = {
      phone,
      message: "📆 *Calendário Anual da Cresci e Perdi*\n\nAcesse aqui: https://crescieperdi.com.br/calendario\n\n📅 Você encontrará:\n• Datas importantes\n• Eventos da franquia\n• Calendário promocional\n• Feriados e comemorações",
    };

    const zapiUrl = `${baseUrl}/instances/${instanceId}/token/${instanceToken}/send-text`;
    console.log(`📤 Enviando calendário info para Z-API: ${zapiUrl.replace(instanceToken, '****')}`);
    ...
    console.log("📤 Calendário info enviado:", data);
    ...
    console.error("❌ Erro no autoatendimento_calendario:", err);
    return new Response(
