...
serve(async (req) => {
  ...
  try {
    ...
    if (buttonId.startsWith("responder_ticket_")) {
      ...
      // ✅ SLA NÃO É MAIS PAUSADO quando franqueado responde
      // Mantém apenas pausa por horário comercial (sla_pausado_horario)
      console.log(`📝 Estado salvo: aguardando resposta do franqueado (ticket #${ticket.codigo_ticket})`);
