...
export const NotificationButton = ({
  ...
  const getAlertTypeLabel = (type: string) => {
    switch (type) {
      ...
      case 'sla_half':
        return 'SLA 50%';
      case 'ia_escalation_crisis':
        return 'IA Escalou Crise';
      case 'critical_ai_response':
        return 'IA Resposta Crítica';
      case 'internal_access_request':
        return 'Solicitação de Acesso';
      case 'crisis_resolved':
        return 'Crise Resolvida';
      case 'crisis':
        return 'Crise Ativa';
      case 'crisis_update':
        return 'Atualização de Crise';
      case 'ticket_created':
        return 'Novo Ticket';
      default:
  ...
  if (loading) {
    return <Button variant="ghost" size="sm" disabled className="text-foreground hover:bg-primary/10 hover:backdrop-blur-sm transition-all duration-450">
  ...
  return <Popover open={open} onOpenChange={setOpen}>
      ...
      <PopoverContent className="w-80 p-0" align="end">
        <div className="p-4 pb-0">
          ...
          }} className="h-8 w-8 p-0" title="Testar som das notificações">
        ...
        <ScrollArea className="max-h-80">
          {alerts.length === 0 ? <div className="p-4 text-center text-sm text-muted-foreground">
              <CheckCircle className="h-8 w-8 mx-auto mb-2 opacity-50" />
              <p>Nenhuma notificação</p>
            </div> : <div className="space-y-1">
              {alerts.slice(0, 10).map(alert => <div key={alert.id} className="p-3 bg-white hover:bg-white/80 transition-colors cursor-pointer border-b last:border-b-0 mx-2 rounded-lg mb-2 shadow-sm">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        <Badge variant="outline" className="text-xs">
                          {alert.alert_level === 'critical' ? 'Crítico' : alert.alert_level === 'warning' ? 'Aviso' : 'Normal'}
                        </Badge>
                      </div>
                          ...
                          locale: ptBR
                            ...
                            <div className="text-xs">
                              <span className="text-primary font-medium">
                                {alert.tickets?.titulo || (alert.tickets?.descricao_problema && alert.tickets.descricao_problema.length > 50 ? alert.tickets.descricao_problema.substring(0, 50) + '...' : alert.tickets?.descricao_problema) || 'Ticket sem título'}
                              </span>
                            </div>
                          </> : <div className="flex items-center gap-2">
                            <span className="font-mono bg-muted px-1 rounded">
                      ...
                      <div className="flex items-center gap-2 mt-2">
                        {alert.ticket_id ? <Button variant="ghost" size="sm" className="h-6 text-xs" onClick={() => {
                    ...
                    if (currentPath === '/admin/tickets') {
                      ...
                      window.location.href = `/admin/tickets?ticket=${alert.ticket_id}`;
                            ...
                            Ver Ticket
                          </Button> : <Button variant="ghost" size="sm" className="h-6 text-xs" onClick={() => {
                    setOpen(false);
                    navigate('/admin');
                  }}>
                            Ver Painel
                          </Button>}
                        <Button variant="ghost" size="sm" className="h-6 text-xs" onClick={() => markAlertAsProcessed(alert.id)}>
                          Marcar Lido
                        </Button>
                      </div>
                    </div>
